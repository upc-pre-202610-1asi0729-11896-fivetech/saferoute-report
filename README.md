...
| TS1 | API: Obtener Alumnos | Como developer, quiero listar alumnos por ruta, para integraciones externas. | **S1:** Given GET `/students`, When token Admin, Then retorna JSON lista. <br> **S2:** Given filtro por ID, When consulta, Then retorna datos de 1 alumno. <br> **S3:** Given token expirado, When consulta, Then retorna 401 Unauthorized. | E5 |
| TS2 | API: Registro GPS | Como developer, quiero enviar coordenadas, para actualizar la posición del bus. | **S1:** Given alta frecuencia, When envía 10 req/seg, Then el sistema procesa sin delay. <br> **S2:** Given coordenadas inválidas, When envía null, Then retorna 400 Bad Request. | E5 |
| TS3 | API: Gestión de Rutas | Como developer, quiero exponer endpoints para crear y consultar rutas, para integraciones externas. | **S1:** Given POST /routes, When datos válidos, Then retorna 201 Created. <br> **S2:** Given GET /routes/{id}, When ruta existe, Then retorna JSON con paradas. <br> **S3:** Given ID inexistente, When consulta, Then retorna 404 Not Found. | E5 |
| TS4 | API: Registro de Abordajes | Como developer, quiero registrar eventos de abordaje vía API, para sincronizar con sistemas externos. | **S1:** Given POST /boarding, When alumno válido, Then retorna 200 OK. <br> **S2:** Given evento duplicado, When intenta registrar, Then retorna 409 Conflict.<br> **S3:** Given token inválido, When consulta, Then retorna 401 Unauthorized. | E5 |
| TS5 | API: Reporte de Incidencias | CComo developer, quiero enviar incidencias vía API, para integrarlas en dashboards externos. | **S1:** Given POST /incidents, When datos válidos, Then retorna 201 Created. <br> **S2:** Given GET /incidents, When consulta, Then retorna lista filtrada. <br> **S3:** Given payload incompleto, When envía, Then retorna 400 Bad Request. | E5 |
| TS6 | API: Historial de Viajes | Como developer, quiero consultar viajes pasados, para análisis externo. | **S1:** Given GET /trips, When token válido, Then retorna lista de viajes. <br> **S2:** Given filtro por fecha, When consulta, Then retorna viajes del día.<br> **S3:** Given token expirado, When consulta, Then retorna 401 Unauthorized. | E5 |
| TS7 | API: Notificaciones Globales | Como developer, quiero enviar notificaciones push desde un endpoint, para integraciones administrativas. | **S1:** Given POST /notifications, When mensaje válido, Then envía push a todos los padres. <br> **S2:** Given programación futura, When fecha llega, Then dispara automáticamente.<br> **S3:** Given mensaje vacío, When intenta enviar, Then retorna 400 Bad Request. | E5 |
| TS8 | API: Cámara en Vivo | Como developer, quiero acceder al stream de cámara, para integraciones de seguridad. | **S1:** Given GET /camera/live, When plan válido, Then retorna stream activo. <br> **S2:** Given modo nocturno, When consulta, Then activa visión infrarroja.<br> **S3:** Given conexión caída, When intenta acceder, Then retorna 503 Service Unavailable. | E5 |
| TS9 | API: Perfil del Estudiante | Como developer, quiero exponer datos médicos y de perfil, para integraciones con sistemas escolares. | **S1:** Given GET /students/{id}, When alumno existe, Then retorna JSON con datos médicos. <br> **S2:** Given PUT /students/{id}, When datos válidos, Then actualiza correctamente.<br> **S3:** Given campo obligatorio vacío, When guarda, Then retorna 400 Bad Request. | E5 |
